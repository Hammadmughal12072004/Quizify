...
const Dashboard = (CUId) => {
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`${window.location.origin}/exam/${CUId.CUId}`);
    setExamNameStorage(data);
    setIsLoading(false);
  }

  const deleteExam = (id) => {
    axios.delete(`${window.location.origin}/exam/${id}`).then((response) => {
  ...
  const handleName = (e) => {
    ...
    if (examName == "") {
      ...
      console.log(newExam)
      axios.post('${window.location.origin}/exam/', newExam).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
      setDummy(dummy + 1)
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableHead>
                <TableRow style={{ backgroundColor: "whitesmoke" }}>
                  <TableCell><h1>Quizzes</h1></TableCell>
              ...
              <TableBody>
                {examNameStorage.map((name) => (
                  ...
                  >
                    <TableCell component="th" scope="row" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/quiz/` + name._id) }}>
                      <span style={{ cursor: "pointer" }} onClick={() => { notify(); }}> {name.examname}  <span style={{ color: "#CC0000" }}>  </span> </span>
                    </TableCell>
